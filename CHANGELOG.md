...
- `PhysxSceneConfig.gpu_broadphase_num_scenes` and `gpu_broadphase_with_shared_scene` declare the environment layout; `PhysxGpuSystem` reads them at construction and freezes them there, which is the only point where either applies: PhysX bakes the broadphase bit count into the scene at `createScene`, and refuses `PxActor::setEnvironmentID` once an actor is in a scene. `num_scenes` sizes the bit count, hands unset scenes a fresh unique ID, and derives what PhysX receives. `with_shared_scene` offsets every ID into the bands that still overlap objects shared by all environments. The two are independent. With neither, SAPIEN does nothing: IDs reach PhysX verbatim and an untouched scene is environment 0.
- With `with_shared_scene`, SAPIEN also writes each scene's environment ID into the high 16 bits of every shape's fourth collision group word, where its filter shader compares it, so cross-environment pairs are rejected exactly rather than only spread apart by the broadphase. This carries the *raw* environment ID with no band offset, because the field is compared for equality rather than encoded into bounds; shared scenes get `0xffff`. The field is 16 bits, so environment IDs from `0xffff` up are rejected while it is on.
- Added `PhysxSceneConfig.set_gpu_broadphase_env_id_bits(bits)` and `set_gpu_broadphase_env_count(env_count, with_shared_objects=True)` for callers sizing the banding themselves, plus module-level `broadphase_env_id_bits_for_env_count()`, `max_broadphase_env_count()` and `broadphase_env_id_window(bits_x, bits_y, bits_z)` — the last reports where the safe band window is for simulations driving environment IDs by hand.
- Added read-only introspection: `PhysxSceneConfig.gpu_broadphase_env_band_count` and `gpu_broadphase_max_env_count`; on `PhysxSystemGpu` `has_shared_environment_scene`, `has_managed_broadphase_env_ids`, `manages_collision_group_scene_ids`, `broadphase_env_id_window`, `broadphase_env_band_count`, and `get_broadphase_environment_id(scene)` (the ID `PxActor::setEnvironmentID` actually receives).

### Changed

- `gpu_broadphase_env_id_bits` now defaults to **5** instead of 0, written to the **Z axis alone**. Five is the widest count whose every band still reaches the fixed encoded interval PhysX gives shared objects, so the out-of-the-box configuration spreads environments apart without reserving anything; with the banding off entirely, coincident environments measured about 15% slower at 4096. Z-only because the axes are not independent — PhysX shifts the same environment ID on each, so the band count is `2 ** max(x, y, z)` and spreading the bits buys none while costing coordinate precision on every axis touched, since SAPIEN raises PhysX's "snap to grid" shift to match. Assign the per-axis fields directly for another layout.
- The usable band window is now derived from `max(x, y, z)` rather than intersecting the per-axis ranges: the widest axis alone decides how environments are banded, so it alone decides which bands reach shared objects.
- Environments past the usable band window now wrap into the high bits of the ID rather than being rejected. PhysX discards those bits when it places the box but compares the full 32-bit environment ID when it filters the pair — the same `updateData_envIDs` buffer feeds `translateAABBsLaunch` and `filtering()` in `broadphase.cu` — so wrapped environments still collide correctly and share a band only in the broadphase-spreading sense. `capacity` therefore reports how many environments get a private band, not a hard ceiling; under-declaring `num_scenes` only costs private bands. The ceiling is `max_broadphase_env_count`, about 16.6M environments.
- Changing a scene's environment ID now drops the broadphase ID cached for it. Without that, a scene whose ID was read before being changed kept the stale value and two environments could be handed the same one, which made them collide.
- A bit count leaving no band inside the shared interval — one bit, where the boundary between the two bands falls inside it — now raises instead of dividing by zero. Declaring a scene count never reaches this, since the sizing always picks a width that holds it.
- Marking a scene shared without having declared `with_shared_scene` warns, because no bands were reserved for it; declaring it and never using one warns at `gpu_init`.
- Documented that the three per-axis environment-ID bit counts are not independent. PhysX shifts the same environment ID on every axis, so each axis keeps the low bits of one value: `(6, 0, 0)` behaves exactly like `(6, 6, 6)` and the distinct band count is `2 ** max(x, y, z)`, never the product. Environments beyond that count share bands, which costs broadphase spreading but stays correct, because PhysX filters candidate pairs on the full environment ID.
