...
docker run -v `pwd`:/workspace/SAPIEN -it --rm \
       ...
       yolkarian/sapien-build-env:0.3 bash -c "export CMAKE_BUILD_PARALLEL_LEVEL=${CMAKE_BUILD_PARALLEL_LEVEL} && cd /workspace/SAPIEN && ./scripts/build.sh ${PYTHON_VERSION} --debug"
