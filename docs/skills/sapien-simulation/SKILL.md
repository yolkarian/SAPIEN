...
description: Build, refactor, or review SAPIEN simulation environments, especially PhysX GPU setup, scene/build/light/render/sensor initialization order, GPU buffer access, batched rendering, custom GPU IK from PhysX Jacobian buffers, reset workflows, step workflows, and Docker container packaging of GPU PhysX and rendering dependencies.
---
...
12. For custom batched GPU IK, build articulations into a `Scene` owned by the target `PhysxGpuSystem`, call `gpu_init()`, then use low-level buffers such as `cuda_articulation_link_data`, `cuda_articulation_jacobian`, and `cuda_articulation_jacobian_shape`. SAPIEN no longer exports `GpuInverseKinematicsSolver` or `gpu_inverse_kinematics`.
...
- Do not use `sapien.physx.GpuInverseKinematicsSolver` or `sapien.physx.gpu_inverse_kinematics`; they are removed. Build custom IK from low-level PhysX GPU buffers instead.
