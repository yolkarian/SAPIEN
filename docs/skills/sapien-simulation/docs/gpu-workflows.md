...
- Prefer PhysX GPU scene environment IDs over collision-group hacks for multi-env isolation. Declare the mode on `PhysxSceneConfig` BEFORE constructing `PhysxGpuSystem` (read once and frozen): managed mode (`num_scenes = N`, a hard cap on ordinary scenes — assigning one more raises) has SAPIEN assign every Scene a unique ID via `Scene.get_or_assign_environment_id()`; manual mode (`num_scenes` unset) has you number scenes by hand with `Scene.set_environment_id(env_id)` before the scene gets any body — duplicates are legal and put several Scenes in one environment. In both modes, `with_shared_scene = True` plus `Scene.set_shared_environment()` on the one shared scene (before it gets a body; reports `environment_id == -1`, costs no `num_scenes` slot) shifts every ID into the band that reaches the shared object. Each method raises in the wrong mode. `Scene.environment_id` is the raw ID; `PhysxGpuSystem.get_broadphase_environment_id(scene)` is the shifted value PhysX receives.
- In GPU runtime, update state through `sapien.physx.PhysxGpuSystem.cuda_*` buffers plus `gpu_apply_*()`.
- Read state only after the needed `gpu_fetch_*()` calls. Consume or clone fetched CUDA tensors on the stream configured by `gpu_set_cuda_stream`; synchronize it explicitly before host/cross-stream access. Fetch scratch reuse is ordered after earlier configured-stream consumers with CUDA events, not host-wide waits (including the unreleased rigid-body/link-pose ordering fix). Do not rely on a joint-first fetch sequence or `CUDA_LAUNCH_BLOCKING=1` to hide missing stream dependencies.
- Contact impulse queries synchronize by default. With `synchronize=False`, consume `query.cuda_impulses` only on the stream configured by `gpu_set_cuda_stream`, or call `gpu_wait_contact_queries()` before host/cross-stream access. `step()` / `step_start()` wait for outstanding asynchronous queries before PhysX invalidates prior contact pointers.
...
- For direct GPU camera/offscreen rendering, use GPU pose batch indices with `sapien.render.RenderSystemGroup.set_cuda_poses(physx_system.cuda_rigid_body_data)` and read images through `get_picture_cuda(...)` instead of syncing poses to CPU. `RenderSystemGroup.update_render()` does not fetch PhysX state: after manually applying articulation root/qpos and calling `gpu_update_articulation_kinematics()`, call `gpu_fetch_articulation_link_pose()` so the public link rows in `cuda_rigid_body_data` are current before rendering.
...
6. For multiple scenes sharing one `PhysxGpuSystem`, declare the layout on `PhysxSceneConfig` before creating the system, which reads and freezes it:
   - Managed mode: `num_scenes = N` — how many ordinary environments. SAPIEN then gives each scene a unique env ID (lazily, or on `scene.get_or_assign_environment_id()`) and derives the broadphase bit counts; `N` is a hard cap, so assigning one more raises, and slots are returned when a scene is destroyed or turned shared. `Scene.set_environment_id()` raises in this mode.
   - Manual mode: leave `num_scenes` unset. SAPIEN assigns nothing; an unnumbered scene is environment 0. Number scenes yourself with `scene.set_environment_id(env_id)` before the scene gets any actor/articulation (a body freezes its env ID at `PxScene::addActor`). `env_id` is the raw, unshifted ID — `scene.environment_id` reads back exactly what was passed. Duplicates are legal and meaningful: giving several Scenes the same ID puts them in one environment so they collide with each other, which auto-assignment cannot express. `get_or_assign_environment_id()` raises in this mode.
   - Construction is the only point where either mode applies — PhysX bakes the bit count into the scene at `createScene` and refuses `PxActor::setEnvironmentID` once an actor is in a scene.
   - `scene.environment_id` is read-only: the raw, unshifted ID SAPIEN's filter shader compares (a dense non-negative int, `-1` for a shared scene, `None` until assigned). It is NOT the broadphase effective ID; `physx_system.get_broadphase_environment_id(scene)` is the value `PxActor::setEnvironmentID` receives, with the band offset applied only there — without a shared scene the offset is 0 so the two match, with one they differ.
   - `config.set_gpu_broadphase_env_id_bits(x, y, z)` is the only way to write the bit counts, and is ignored once `num_scenes` is set. Default `(0, 0, 4)`: four matches PhysX's own "snap to grid" shift, so the banding costs no coordinate precision. The axes are not independent — PhysX shifts the same env ID on each, so distinct bands are `2**max(x, y, z)`, never the product — hence one axis is as good as three and cheaper.
   - With `with_shared_scene`, every non-zero count must be equal: a narrower axis keeps fewer of the shifted ID's low bits, so an env inside the widest axis's usable band wraps out of it there and silently loses the shared object. SAPIEN rejects mixed counts like `(12, 8, 0)` at construction and widens `(0, 0, 4)` to `(4, 4, 4)`.
   - PhysX bands each env into `[e << (32 - b), (e + 1) << (32 - b))` but gives shared objects a fixed encoded interval the outermost bands fall outside of, so with a shared scene a bit count that exactly fits is one bit too small: 4096 envs need 13 bits. Read `physx_system.get_broadphase_environment_id(scene)` for the ID PhysX actually receives. Declaring more envs than get private bands is fine — the surplus wraps into the high bits, which PhysX drops when placing the box but compares exactly when filtering. With a shared scene the hard ceiling is 65535 ordinary envs, since SAPIEN keeps the scene ID in a 16-bit collision-group field and reserves `0xffff` for the shared scene.
   - Use `physx_system.set_scene_offset(scene, offset)` when spatial separation is needed for visualization, or as the only isolation when `num_scenes` is not declared.
...
11. Fetch initial state, write initial GPU buffers, then apply:
    ...
    - `gpu_fetch_articulation_link_pose()` before a Viewer/sensor consumes `cuda_rigid_body_data`
...
- Direct GPU camera/offscreen rendering, including one-scene capture:
  ...
  2. For free cameras created with `scene.add_camera(...)`, set the initial world transform in exactly one place before `gpu_init()`, usually `camera.set_local_pose(sapien.Pose(p=..., q=...))` while the owning entity remains identity. `gpu_init()` seeds a `'cuda'`-mode camera's pose row from this pose; for `'static'` (the free-camera default) and `'cpu'` the CPU pose stays authoritative.
  3. Create one `RenderSystemGroup([scene.get_render_system(), ...])`, call `set_cuda_poses(physx_system.cuda_rigid_body_data)`, create every `RenderCameraGroup`, configure each free camera's pose mode with `camera_group.set_pose_mode(camera, "cuda"|"cpu"|"static")`, then call `render_system_group.gpu_init()` exactly once. Camera groups cannot be created after `gpu_init()`, and `set_pose_mode`/steady-state calls before it raise. Mounted cameras on PhysX GPU bodies are auto-attached and cannot be configured.
  ...
  5. For a fixed or moving free camera (raster or RT), use this order every frame:
     1. If the frame manually restores articulation root/qpos state rather than following a completed simulation step, apply it, call `gpu_update_articulation_kinematics()`, then `gpu_fetch_articulation_link_pose()`. The update invalidates the public link rows and the fetch repopulates the `cuda_rigid_body_data` source bound to the render group. A normal simulation step still needs its usual post-step fetches before rendering.
     2. When a `'cuda'`-mode camera moves, write its world pose row `[px, py, pz, qw, qx, qy, qz]` in `camera_group.cuda_poses` on the GPU, or copy one CPU-authored pose explicitly with `camera_group.set_cuda_pose(camera, pose)`. For a `'cpu'`-mode host-driven camera, call `camera.set_local_pose(...)`; the pose uploads at the next `update_render()`. `'static'` cameras do not move.
     3. Call `render_system_group.update_render()`; it refreshes CPU components (raising on static-pose tampering), uploads dirty CPU camera/scene-light state, then writes the already-fetched PhysX CUDA body poses and grouped camera buffers with bidirectional Vulkan/CUDA semaphore ordering. It does not fetch PhysX state implicitly.
     4. Run `camera.take_picture()` and keep `camera.get_picture_cuda(name)`, or capture through `camera_group.take_picture()`; both read the same GPU-owned transforms.
  ...
  camera_group.set_pose_mode(camera, "cuda")  # after create, before gpu_init()
  render_group.gpu_init()

  # Each free-camera frame (cuda mode): one pose-row write, one ordered update, one capture.
  camera_group.set_cuda_pose(camera, updated_pose)  # or write camera_group.cuda_poses
  render_group.update_render()   # ordered CUDA body/link + camera update
  camera.take_picture()
  color_cuda = camera.get_picture_cuda("Color")

  # Alternative for a host-driven follow camera: set_pose_mode(camera, "cpu"), then per
  # frame call camera.set_local_pose(follow_pose); update_render() uploads it.
  ```
...
- After `gpu_init()`, real-time light properties remain CPU-settable on raster and RT: color, spot inner/outer FOV, parallelogram shape, shadow near/far, directional shadow half-size, and ambient light. `'cpu'`-mode lights also propagate local/entity pose each frame and upload when dirty; `'static'`-mode lights (default) raise on a pose move after `gpu_init()` but still allow color/FOV/shape/shadow-parameter changes. Setup-only fields that raise after `gpu_init()`: shadow enable/disable, shadow map size, and textured-light texture.
- Viewer path:
...
8. Fetch state with the needed `gpu_fetch_*()` calls. Always include `gpu_fetch_articulation_link_pose()` before rendering or reading articulation-link rows from `cuda_rigid_body_data`; the kinematics update invalidates those public rows.
...
- Prefer PhysX GPU environment IDs when available:
  - Managed mode: declare `PhysxSceneConfig.num_scenes` before creating the system; SAPIEN then assigns each scene a unique env ID, at the latest when its first body is added. `set_environment_id()` raises; duplicates cannot occur. Manual mode (`num_scenes` unset): number scenes yourself with `scene.set_environment_id(env_id)` before the scene gets any body; an unnumbered scene is env 0, and duplicates are legal — several Scenes sharing one ID collide as one environment.
  - Bodies with the same non-negative env ID collide with each other; `environment_id` is the raw ID, `get_broadphase_environment_id(scene)` the shifted value PhysX receives.
  - Declare `with_shared_scene` and call `scene.set_shared_environment()` on the one scene holding bodies every env must collide with, such as a single ground plane. It works in both modes, reports `environment_id == -1`, and costs no `num_scenes` slot.
...
- Calling `gpu_update_articulation_kinematics()` after replay/reset state restoration and rendering immediately without `gpu_fetch_articulation_link_pose()`. Rigid actors written directly into `cuda_rigid_body_data` will move while articulation links remain visually stale because `RenderSystemGroup` performs no implicit fetch.
...
- Calling `set_local_pose()` on a `'static'` or `'cuda'`-mode camera in a `RenderCameraGroup`; `gpu_init()` seals those camera transforms for GPU ownership and the setter raises. Use `'cpu'` mode for host-driven cameras, or move `'cuda'`-mode cameras through `camera_group.cuda_poses`/`set_cuda_pose`.
...
- Calling `update_render()` or capturing before `RenderSystemGroup.gpu_init()`, or creating camera groups or calling `set_pose_mode` after it; the lifecycle is configure (including pose mode), `gpu_init()`, then steady state.
- Moving a CPU-owned static body (no CUDA pose source), point cloud, or `'static'`-mode light after `gpu_init()`; those transforms are sealed one-time snapshots and tampering raises at `RenderSystemGroup.update_render()` (or the next `scene.update_render()` for entity-pose changes).
