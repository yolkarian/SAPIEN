...
6. For multiple scenes sharing one `PhysxGpuSystem`, isolate envs before adding any PhysX body:
   ...
   - Declare the layout on `PhysxSceneConfig` before creating the system, which reads and freezes it: `gpu_broadphase_num_scenes = N` sizes the bit count, hands unset scenes a fresh unique ID, and derives what PhysX receives; `gpu_broadphase_with_shared_scene = True` offsets every ID into the bands that still overlap shared objects and turns on SAPIEN's own collision-group filter (raw env ID in the high 16 bits of collision group word 3, `0xffff` for shared). The two are independent. Construction is the only point where either applies — PhysX bakes the bit count into the scene at `createScene` and refuses `PxActor::setEnvironmentID` once an actor is in a scene. Neither set means SAPIEN does nothing: IDs reach PhysX verbatim and an untouched scene is environment 0.
   - `gpu_broadphase_env_id_bits` defaults to **5**, written to **Z alone**. The axes are not independent — PhysX shifts the same env ID on each, so distinct bands are `2**max(x, y, z)`, never the product — hence spreading the bits buys no bands and costs coordinate precision on each axis touched, since SAPIEN raises PhysX's "snap to grid" shift to match. Five is the widest count whose every band still reaches shared objects, so the default needs no offset. Assign the per-axis fields directly to use another axis.
   - PhysX bands each env into `[e << (32 - b), (e + 1) << (32 - b))` but gives shared objects a fixed encoded interval the outermost bands fall outside of, so with a shared scene a bit count that exactly fits is one bit too small: 4096 envs need 13 bits. Read `physx_system.broadphase_env_id_window` for the `(offset, capacity)` in force, `broadphase_env_band_count` for the distinct-band count, and module-level `sapien.physx.broadphase_env_id_window(x, y, z)` when driving IDs by hand — it is derived from `max(x, y, z)`. `capacity` is how many envs get a private band, not a ceiling: envs past it wrap into the high bits, which PhysX drops when placing the box but compares exactly when filtering, so under-declaring only costs private bands. The ceiling is `max_broadphase_env_count` (~16.6M).
   - With `bits = 0` the env-ID branch is skipped entirely: no banding, and shared objects keep their real bounds rather than the fixed interval. Correct but with zero spreading — measured ~15% slower at 4096 coincident envs.
   - Set a scene's env ID before adding any body to it; both SAPIEN and PhysX refuse the change afterwards. Marking a scene shared without having declared it warns, as does declaring it and never using one.
